...
// but don't color-code it." Every other stage uses the dealership's own
...
export function isAgingRed(stage: string, days: number, yellowDays: number, redDays: number): boolean {
  const thresholds = getThresholds(stage, yellowDays, redDays);
